...
My baseline model is a mean home goals and mean away goals model, I calculated the means using the training set and evaluated the result with the test data set.The baseline result is `r RMSE_baseline_onesplit`.

The next model was a simple generalized linear model, trained with the train set and evaluated with the test set. The improvement here was small, the RMSE was `r RMSE_glm_onesplit`. The glmnet model is an evolution of the glm model, but here it gave the same result as the glm model, `r RMSE_glmnet_onesplit`.

A random forest model seems like a good modeling solution. I set up the random forest using the following code:

```{r, eval=FALSE, warning=FALSE, echo=TRUE}
train(Goals ~ Home + WeekNumber + PointsDifference + ValueDifference + 
...
I varied ntree to find an optimum solution as shown in the chart below. Although the RMSE is lower than the baseline model, it's still not a good solution. The lowest RMSE I found for my random forest model was `r rf_plot_onesplit`, which means the error is just less than a goal, not a great result and not much of an improvement over a simple linear model.
...
Finally, I tried a Support Vector Machine with the following code.
...
Once again, this model didn't perform well, giving a minimum RMSE of `r RMSE_svm_onesplit`.
ACTION - TWEAK CODE !!!!!!!!

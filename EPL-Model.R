...
RMSE_baseline <- RMSE(test$Goals, mean(train$Goals))
print(sprintf("Baseline RMSE %s", RMSE_baseline))
...
RMSE_glm <- RMSE(test$Goals, predict_glm)
print(sprintf("glm %s", RMSE_glm))
...
RMSE_glmnet <- RMSE(test$Goals, predict_glmnet)
print(sprintf("glmnet %s", RMSE_glmnet))
...
rf_model <- function(ntree) {
  ...
  print(sprintf("ntree = %f, RMSE = %f", ntree, RMSE_rf))
}
ntrees <- seq(from=10, to=200, by=10)
for (ntree in ntrees) { 
  rf_model(ntree)
}
...
RMSE_svm <- RMSE(test$Goals, predict_svm)
print(sprintf("svm %s", RMSE_svm))
